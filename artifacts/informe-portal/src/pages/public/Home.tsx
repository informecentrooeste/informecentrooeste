        ...
        <div className="w-full lg:w-[70%] flex flex-col gap-10">
          ...
          <section>
            <div className="flex items-center mb-4">
              <span className="bg-red-600 text-white text-xs font-black px-3 py-1 rounded mr-3 flex items-center gap-1.5 animate-pulse shadow-sm">
                <span className="w-2 h-2 bg-white rounded-full"></span> AO VIVO
              </span>
              <h2 className="text-2xl font-black text-primary tracking-tight">TV INFORME</h2>
            </div>
            <div className="w-full aspect-video rounded-2xl overflow-hidden shadow-xl border-4 border-gray-900 bg-black">
              <iframe src="https://player.logicahost.com.br/player.php?player=2050" className="w-full h-full border-0" allowFullScreen></iframe>
            </div>
          </section>

          {/* 2. BANNER PROPAGANDA */}
          <div className="w-full bg-gray-100 h-[90px] flex items-center justify-center text-gray-400 font-bold text-sm rounded-xl border-2 border-dashed border-gray-200">
            BANNER PROPAGANDA
          </div>
          ...
          {featured && featured.length > 0 && (
            <section>
              <h2 className="text-2xl font-black border-l-[6px] border-red-600 pl-4 mb-6 text-primary uppercase tracking-tight">Destaques</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {/* FORMIGA */}
            <section>
              <div className="flex items-center justify-between border-b-2 border-border mb-5 pb-2">
                <h2 className="text-2xl font-black border-l-[6px] border-primary pl-3 text-primary uppercase">Formiga</h2>
              </div>
            ...
            <section>
              <div className="flex items-center justify-between border-b-2 border-border mb-5 pb-2">
                <h2 className="text-2xl font-black border-l-[6px] border-primary pl-3 text-primary uppercase">Regional</h2>
              </div>
              <div className="flex flex-col gap-5">
          ...
          <section className="bg-card p-8 rounded-2xl shadow-lg shadow-black/5 border border-border">
            <h2 className="text-2xl font-black border-l-[6px] border-primary pl-3 mb-8 text-primary uppercase">Articulistas</h2>
            <div className="flex gap-6 overflow-x-auto pb-4 scrollbar-hide snap-x">
              {['João Silva', 'Maria Clara', 'Pedro Santos', 'Ana Beatriz', 'Carlos Eduardo', 'Luciana Costa'].map((name, i) => (
                <div key={i} className="flex flex-col items-center gap-3 cursor-pointer group min-w-[110px] snap-start">
                  <div className="w-24 h-24 rounded-full bg-gradient-to-br from-primary to-accent p-1 transition-transform duration-300 group-hover:-translate-y-2 group-hover:shadow-xl">
                    <div className="w-full h-full bg-white rounded-full border-4 border-white overflow-hidden flex items-center justify-center">
                      <div className="text-2xl font-black text-primary/40">{name.charAt(0)}</div>
                    </div>
                  </div>
                  <span className="font-bold text-sm text-center group-hover:text-primary transition-colors">{name}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          {/* 9. POLÍTICA SECTION */}
          <section>
            <h2 className="text-2xl font-black border-l-[6px] border-primary pl-3 mb-6 text-primary uppercase">Política</h2>
          ...
          {/* 10. GERAL SECTION */}
          <section>
            <h2 className="text-2xl font-black border-l-[6px] border-primary pl-3 mb-6 text-primary uppercase">Geral</h2>
          ...
          <section className="bg-primary p-8 rounded-2xl text-primary-foreground mb-8 shadow-xl relative overflow-hidden">
            <div className="absolute top-0 right-0 p-8 opacity-10 rotate-12 scale-150">
              <Instagram className="w-64 h-64" />
            </div>
            <h2 className="text-2xl font-black flex items-center gap-3 mb-8 uppercase relative z-10">
              <Instagram className="h-8 w-8" /> Vídeos do Instagram
            </h2>
            
            <div className="flex gap-5 overflow-x-auto pb-4 scrollbar-hide snap-x relative z-10">
              {[1, 2, 3, 4, 5, 6].map((item) => (
                <div key={item} className="snap-start shrink-0 w-40 aspect-[9/16] bg-black rounded-xl overflow-hidden relative group cursor-pointer border-2 border-white/20 hover:border-white/50 transition-colors shadow-lg">
